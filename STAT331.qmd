...
```{r}
colnames(summarized_country)
colnames(data)
str(summarized_country)
data<- data |> 
  clean_names()

summarized_country <- summarized_country |> 
...
models <- list(
     "Simple Linear (Mean Spending, No Log)"  = lm(mean_life_expectancy ~ mean_spending, data = summarized_country),
   "Simple Linear (Mean Spending, Log)"     = lm(mean_life_expectancy ~ log(mean_spending), data = summarized_country),
    "Simple Linear (Yearly Spending, No Log)"= lm(life_expectancy ~ spending, data = data),
  "Simple Linear (Yearly Spending, Log)"   =  lm(life_expectancy ~ log(spending), data = data))

model_comparison <- enframe(models, name = "Model", value = "Fit") |> 
...
model_comparison <- enframe(models, name = "Model", value = "Fit") |> 
  mutate("R-Squared" = map_dbl(Fit, ~ summary(.x)$r.squared)) |> 
  select(-Fit)


kable(model_comparison, format = "html", caption = "Model R-Squared Comparison") |> 
...
models <- list(
  ...
  "Model 3(Yearly Spending, No Log)"= lm(life_expectancy ~ spending, data = data),
  "Model 4 (Yearly Spending, Log)"   = lm(life_expectancy ~ log(spending), data = data))
...
kable(final_model, format = "html", caption = "Regression Models") |> 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
```{r}
datatable(final_model, options = list(pageLength = 10, autoWidth = TRUE),
          caption = HTML("<h3 style='text-align:center; font-weight:bold;'>Table: Model </h3>")) |> 
  formatRound(columns = c("Variance in Response Values", "Variance in Fitted Values", "Variance in Residuals", "R-Squared"), digits = 4)
```

```{r}
caption = htmltools::HTML(kable_html))

```

```{r}
# Generate kable HTML table
kable_html <- final_model %>%
  kable("html", caption = "<b>Final Model Results</b>") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))

# Embed kable inside datatable
datatable(as.data.frame(final_model), 
          escape = FALSE, 
